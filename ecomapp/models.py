...
class UserProductRating(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    product = models.ForeignKey(Product, on_delete=models.CASCADE)
    rating = models.FloatField(default=0.0)  # 1.0 to 5.0
...
# 🚀 Auto-Fill Ratings when an Order is created
@receiver(post_save, sender='ecomapp.Order')
def create_ratings_on_order(sender, instance, created, **kwargs):
    if created:
        cart_products = instance.cart.cartproduct_set.all()
        user = instance.cart.customer.user
        for cart_product in cart_products:
            UserProductRating.objects.get_or_create(
                user=user,
